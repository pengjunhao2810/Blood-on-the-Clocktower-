...
# 自我升级引擎
from games.blood_on_clocktower.self_upgrader import get_upgrader
...
class GameController:
    ...
    def _init_game(self):
        ...
        # 应用自我升级参数
        self.game._upgrade_params = _upgrader.apply_to_game(self.game)
    ...
    def step(self):
        ...
        if self.finished or self.game.game_record.get('result'):
            self.finished = True
            # 自我升级：游戏结束时自动评估+学习
            if not getattr(self, '_upgraded', False):
                self._upgraded = True
                score, issues, bonuses = _upgrader.evaluate_game(self.game.game_record)
                _upgrader.learn_from_game(self.game.game_record, score)
                # 元脑离线分析：检测错误模式→生成修复建议→进化Prompt
                from games.blood_on_clocktower.meta_brain import get_brain
                brain = get_brain()
                patches = brain.post_game_analysis(self.game.game_record)
    ...
    def get_state(self):
        ...
        # 私聊历史整理成前端友好的格式（同对玩家跨天合并在一个线程）
        chat_threads = []
        pch = g.game_record.get("private_chat_history", {})
        for thread_key, msgs in pch.items():
            # thread_key格式: 玩家1🔄玩家2
            pair_label = thread_key
            # 从消息中的天标记提取最新天数
            day_label = ""
            for m in reversed(msgs):
                if m.get("speaker") == "__day__":
                    day_label = m.get("text", "").replace("第", "第").replace("天", "天")
                    if "第" in day_label:
                        break
            if not day_label:
                day_label = "进行中"
...
<div class="container">
    ...
    <div class="controls">
        ...
        <button class="btn-export" onclick="exportChat()">💬 导出私聊</button>
        <button class="btn-export" onclick="exportThoughts()">🧠 导出思考</button>
    </div>
...
function exportThoughts() { window.open('/export_thoughts'); }
...
def upgrade_status():
    return jsonify(_upgrader.get_status())

@app.route('/meta_status')
def meta_status():
    from games.blood_on_clocktower.meta_brain import get_brain
    return jsonify(get_brain().get_status())
...
def export_chat():
    ...
    doc.add_heading('血染钟楼 · 私聊记录', level=0).alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph('')

    total_threads = 0
    total_msgs = 0
    if not pch:
        p = doc.add_paragraph('暂无私聊记录')
        p.italic = True
    else:
        for tk in sorted(pch.keys()):
            msgs = pch[tk]
            if not msgs:
                continue
            total_threads += 1
            # 线程标题：玩家 A ↔ 玩家 B（排除__day__消息）
            real_messages = [m for m in msgs if m.get('speaker') != '__day__']
            sp1 = real_messages[0].get('speaker', '?') if real_messages else '?'
            sp2 = ''
            for m in real_messages:
                if m.get('speaker') != sp1:
                    sp2 = m.get('speaker')
                    break
            display = f'{sp1} ↔ {sp2}' if sp2 else sp1
            a, b = sp1, sp2
            p = doc.add_paragraph()
            run = p.add_run(f'【{display}】')
            run.bold = True
            ...
            for m in msgs:
                ...
                if sp == '__day__':
                    ...
                    r_day = p_day.add_run(f'── {txt} ──')
                    r_day.bold = True
                    r_day.font.color.rgb = RGBColor(0xFF, 0xD7, 0x00)
                    r_day.font.size = Pt(12)
                    continue

                p2 = doc.add_paragraph()
                # 人头子：第一发言绿色，第二发言红色
                name_color = RGBColor(0x00, 0x80, 0x00) if sp == a else RGBColor(0x8B, 0x00, 0x00)
                rn = p2.add_run(sp)
                rn.bold = True
                rn.font.color.rgb = name_color
                # 如果有天标记，显示天数
                if current_day:
                    rdaytag = p2.add_run(f' [{current_day}]')
                    rdaytag.font.size = Pt(8)
                    rdaytag.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
                p2.add_run(f': {txt}')
                # 序号
                rs = p2.add_run(f'  [#{seq}]')
                rs.font.size = Pt(8)
    ...
    run = p.add_run(f'共 {total_threads} 组私聊对话，{total_msgs} 条消息')
    run.italic = True
    ...
    buf.seek(0)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    return send_file(buf, as_attachment=True, download_name=f'私聊记录_{ts}.docx',
                     mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')

@app.route('/export_thoughts')
def export_thoughts():
    try:
        g = ctrl.game
    except:
        return jsonify({"error": "游戏未初始化"}), 400
    thought_log = g.game_record.get("thought_log", {})
    doc = Document()
    style = doc.styles['Normal']
    style.font.name = 'Microsoft YaHei'
    ...
    doc.add_heading('血染钟楼 · AI思考日志', level=0).alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph('')

    if not thought_log:
        p = doc.add_paragraph('暂无思考记录（私聊阶段后生成）')
        p.italic = True
    else:
        for day_key in sorted(thought_log.keys()):
            entries = thought_log[day_key]
            doc.add_heading(day_key, level=1)

            for e in entries:
                role = e.get("role", "?")
                player = e.get("player", "?")
                team = e.get("team", "")
                team_label = "🔵善良" if team == "townsfolk" else ("🔴邪恶" if team in ("demon","minion") else "⚪外来者")
                
                p = doc.add_paragraph()
                rn = p.add_run(f'{player} ({role}) {team_label}')
                rn.bold = True
                rn.font.size = Pt(12)

                doc.add_paragraph(f'持有信息: {str(e.get("info", "无"))}')
                doc.add_paragraph(f'信任Top3: {str(e.get("trust_top3", "无"))}')
                doc.add_paragraph(f'嫌疑Top3: {str(e.get("suspect_top3", "无"))}')
                doc.add_paragraph(f'行动规划: {str(e.get("action_plan", "无"))}')
                doc.add_paragraph(f'角色分析: {str(e.get("role_thought", "无"))}')
                if e.get("drunk_check"):
                    doc.add_paragraph(f'容错检查: {str(e.get("drunk_check"))}')
                if e.get("dead_review"):
                    doc.add_paragraph(f'复盘: {str(e.get("dead_review"))}')
                
                p2 = doc.add_paragraph()
                rc = p2.add_run(f'综合判断: {e.get("conclusion", "无")}')
                rc.italic = True

                # 信任/嫌疑详细分数
                trust_dict = e.get("trust_scores", {})
                sus_dict = e.get("suspect_scores", {})
                if trust_dict or sus_dict:
                    t = doc.add_table(rows=1, cols=4)
                    t.style = 'Light Grid Accent 1'
                    for i, h in enumerate(['玩家', '信任分', '嫌疑分', '综合']):
                        t.rows[0].cells[i].text = h
                    all_players = set(list(trust_dict.keys()) + list(sus_dict.keys()))
                    for pn in sorted(all_players, key=lambda x: -(trust_dict.get(x,0)-sus_dict.get(x,0))):
                        row = t.add_row().cells
                        row[0].text = pn
                        row[1].text = str(trust_dict.get(pn, '-'))
                        row[2].text = str(sus_dict.get(pn, '-'))
                        row[3].text = '信任' if trust_dict.get(pn,0) > sus_dict.get(pn,0) else ('怀疑' if sus_dict.get(pn,0) > trust_dict.get(pn,0) else '中立')
                    doc.add_paragraph('')
    ...
    buf.seek(0)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    return send_file(buf, as_attachment=True, download_name=f'AI思考日志_{ts}.docx',
                     mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
...
.day-separator { text-align:center; padding:6px 0; margin:8px 0; }
.day-separator span { background:#2a2a4e; color:#ffd700; padding:3px 16px; border-radius:10px; font-size:12px; font-weight:bold; }
...
function renderThreads(threads) {
    ...
    for(let t of threads) {
        let msgs = t.messages || [];
        let speakers = [...new Set(msgs.filter(m=>m.speaker!=='__day__').map(m => m.speaker))];
        let label = (t.pair || speakers.join(' ↔ ') || '?').replace(/🔄/g, ' ↔ ');
        let avatar = (speakers[0] || label).charAt(0);
        let color = cols[threads.indexOf(t) % cols.length];
        let lastMsg = [...msgs].reverse().find(m => m.speaker !== '__day__');
...
function renderBubbles(thread) {
    ...
    let pair = thread.pair || thread.key || '';
    document.getElementById('chatTitle').textContent = pair.replace(/🔄/g, ' ↔ ') || '对话';
    ...
    let realMsgs = msgs.filter(m => m.speaker !== '__day__');
    let speakers = [...new Set(realMsgs.map(m => m.speaker))];
    ...
    for(let m of msgs) {
        // 天分隔符
        if(m.speaker === '__day__') {
            h += '<div class="day-separator"><span>' + escapeHtml(m.text) + '</span></div>';
            continue;
        }
