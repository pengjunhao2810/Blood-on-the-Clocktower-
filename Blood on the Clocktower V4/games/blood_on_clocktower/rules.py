...
class BloodOnClocktowerGame(GameManager):
    ...
    def run_night(self):
        ...
        new_deaths = list(dict.fromkeys(new_deaths))  # 去重
    ...
    def _imp_kill(self, player):
        ...
        # 自刀只在has_sw_sacrifice路径, 不走valid_targets筛选
    ...
    def _private_chat_phase(self):
        ...
        for name_a, name_b in chat_pairs:
            ...
            for rnd in range(1, chat_rounds + 1):
                ...
                seq += 1
                # 死人名字过滤：替换发言中的已死亡玩家名
                for dead_name in self.dead_players:
                    if dead_name != a.name and dead_name != b.name and dead_name in s_a:
                        s_a = s_a.replace(dead_name, "其他人")
                ...
                # 死人名字过滤
                for dead_name in self.dead_players:
                    if dead_name != b.name and dead_name != a.name and dead_name in s_b:
                        s_b = s_b.replace(dead_name, "其他人")
    ...
    def _validate_speech_kwargs(self, player, kwargs, scene="public"):
        ...
        # 7. 死人过滤：target/suspect/other/other_target 不能是已死亡玩家
        for key in ("target","suspect","other","other_target"):
            val = kwargs.get(key, "")
            if val in self.dead_players:
                alive_others = [p for p in self.get_alive_names() if p != player.name]
                kwargs[key] = alive_others[0] if alive_others else "某人"

        # 8. "身份不明"替换为具体声称
        if "claim" in kwargs and kwargs["claim"] in ("身份不明", "", "?", None):
            _, _, cm = self._build_chat_summary(player)
            target_name = kwargs.get("target", "")
            if target_name and target_name in cm:
                kwargs["claim"] = cm[target_name]
            else:
                kwargs["claim"] = "未公开身份"

        # 9. 防止"X和X之间的互动"自引用
        for key in ("other","other_target"):
            if kwargs.get(key) == kwargs.get("target"):
                alive = [p for p in self.get_alive_names() if p != player.name and p != kwargs.get("target")]
                if alive: kwargs[key] = random.choice(alive)
    ...
    def _gen_private_chat(self, player, listener):
        ...
        if team in {"demon", "minion"}:
            ...
            # 间谍全知：注入全场角色信息用于精准发言
            if effective_role == "间谍" and spy_info_dict:
                # 按威胁排序
                info_priority = ["占卜师","共情者","调查员","洗衣妇","厨师","送葬者","守鸦人","图书管理员"]
                all_roles = list(spy_info_dict.items())
                all_roles.sort(key=lambda x: (0 if x[1] in info_priority else 1))
                top_threats = [f"{n}({r})" for n,r in all_roles[:6] if n != name]
                kwargs["spy_knowledge"] = "已知全场身份:" + "、".join(top_threats)
                # 对好人时：精准知道对方身份，可以假装无意说对细节骗取信任
                if listener and listener in spy_info_dict:
                    kwargs["listener_real_role"] = spy_info_dict[listener]
                    kwargs["claim"] = spy_info_dict[listener]  # 直接知道对方身份
            ...
            if sus > 60:
                ...
                if is_evil_listener:
                    ...
                    # 间谍用全知身份精准发言
                    if effective_role == "间谍" and spy_info_dict:
                        if exchanged == 0 and random.random() < 0.6:
                            cat = "SPY_KNOWLEDGE_TALK"  # 首轮精准点破对方身份
                        else:
                            cat = random.choice(["SPY_PRECISE_ACCUSE", "SPY_KNOWLEDGE_TALK"])
                        t = DD.get_filled(cat, used=used, **kwargs)
                    else:
